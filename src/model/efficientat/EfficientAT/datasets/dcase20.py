...
import pandas as pd
import torch
from datasets.helpers.audiodatasets import (
    PreprocessDataset,
    get_gain_augment_func,
    get_roll_func,
)
...
class MixupDataset(TorchDataset):
    ...
    def __getitem__(self, index):
        ...
        if torch.rand(1) < self.rate:
            ...
            y[y2] = 1.
            y2 = y
            mix_lambda = np.random.beta(self.beta, self.beta)
            mix_lambda = max(mix_lambda, 1. - mix_lambda)
            x1 = x1 - x1.mean()
            x2 = x2 - x2.mean()
            x = (x1 * mix_lambda + x2 * (1. - mix_lambda))
            x = x - x.mean()
            return x, f1, (y1 * mix_lambda + y2 * (1. - mix_lambda)), d1, c1, i1
